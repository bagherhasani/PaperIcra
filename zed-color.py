...
class OsNetReID:
    ...
    def close(self):
        """Release TensorRT/CUDA objects while the CUDA context is still valid."""
        ...
        try:
            ...
            try:
                ...
                pass
            # Delete TensorRT and CUDA objects before detaching the context.
    ...
    def extract(self, frame_bgr: np.ndarray, bbox) -> np.ndarray | None:
        ...
        try:
            ...
            # Never let TensorRT/PyCUDA failures kill robot following; caller falls back to color/position.
            self._failed = True
            return None
        finally:
            if pushed:
...
class ZedFollower(Node):
    def __init__(self):
        ...
        self.min_distance = 0.5    # stop if closer than this [m]
        # Reduced speeds (safer/slower)
        self.max_lin_speed = 0.8   # maximum forward speed [m/s]
        self.max_ang_speed = 0.9   # maximum turn speed [rad/s]
        ...
        self.relock_max_dist_m = 2.5       # soft position memory radius for reacquisition scoring
        self.color_only_relock_max_dist_m = 0.8  # strict fallback when Re-ID is unavailable
        self.relock_score_margin = 0.35    # min score gap between top two candidates (ambiguity rejection)
        ...
        self.reid_min_reacquire = 0.72     # relock requires a real appearance match
        self.reid_strong_reacquire = 0.82  # strong match can overcome weaker position continuity
        self.reid_discrimination_margin = 0.12  # top candidate must outscore 2nd by this much; else refuse
        self.reid_update_interval = 8      # refresh target fingerprint every N confident frames
        self.reid_ema_alpha = 0.05         # slow update prevents identity drift
        self.same_id_reid_check_interval = 3  # same ZED id still gets verified every N frames
        self.same_id_min_reid = 0.68       # below this, assume the ZED id may have transferred
        self.same_id_max_position_jump_m = 1.0  # reject impossible same-id jumps after occlusion
        self._same_id_reid_fail_streak = 0
        ...
        # PERFORMANCE keeps follower FPS responsive; NEURAL is slower and not needed for person following.
        ...
        self.obj_runtime.detection_confidence_threshold = 70
        ...
        self.shape_min_half_pixels = 80      # min colored pixels in each half
        self.shape_min_total_pixels = 300    # min colored pixels total
        self.shape_min_vert_span = 0.18      # fraction of ROI height spanned by colored pixels
    ...
    def _torso_indices(self, keypoints_2d):
        """Return shoulder/hip indexes for the active ZED body format."""
        parts = sl.BODY_18_PARTS if len(keypoints_2d) <= 18 else sl.BODY_38_PARTS
        return (
            parts.LEFT_SHOULDER.value,
            parts.RIGHT_SHOULDER.value,
            parts.LEFT_HIP.value,
            parts.RIGHT_HIP.value,
        )

    def _torso_polygon_from_keypoints(self, keypoints_2d, img_w, img_h):
        """Build a torso polygon (shoulders+hips) from ZED body keypoints."""
        LS, RS, LH, RH = self._torso_indices(keypoints_2d)
...
def draw_person_overlay(img, best_body, best_poly, logical_id,
    ...
    # ID badge above the torso polygon, independent of BODY_18/BODY_38 indexes.
    nx = int(np.mean(best_poly[:, 0]))
    ny = int(np.min(best_poly[:, 1]))
...
def main():
    ...
    window_name = "ZED Person Follow (Color)"
    ...
    while True:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            if node.reid is not None and (
                ...
                node.get_logger().warn("OSNet re-ID disabled after TensorRT/CUDA error; using color + position only.")
                node.reid.close()
                node.reid = None
            ...
            if node.person_mode and node.body_enabled:
                ...
                if node.locked_id is not None:
                    ...
                    # If numeric ID disappeared, keep all bodies as candidates.
                    # Re-ID + color + position continuity below decides; hard position
                    # filtering made green Re-ID matches get ignored while the robot searched.
                ...
                for b in candidates:
                    ...
                    # Color is the primary identity cue. For the exact locked ZED id we
                    # allow temporary color flicker; for registration/relock it must pass.
                    person_mask = None
                    try:
                        m = b.mask
                        if m is not None:
                    ...
                    if not locked_present:
                        ...
                        if node.shirt_shape_filter:
                            if total_col < node.shape_min_total_pixels:
                                continue
                            if left_col < node.shape_min_half_pixels or right_col < node.shape_min_half_pixels:
                                continue
                            if span_frac < node.shape_min_vert_span:
                                continue
                    ...
                    center_x = float(np.mean(poly[:, 0])) if poly is not None else img_w * 0.5
                    center_err = abs(center_x - (img_w * 0.5)) / max(1.0, img_w * 0.5)
                    center_score = max(0.0, 1.0 - center_err)
                    score = 5.0 if locked_present else (2.0 * ratio + 0.4 * center_score - 0.10 * dist)

                    # In relock mode, use last known 3D position as a soft score, not a hard gate.
                    pos_score = 0.0
                    pos_dist = None
                    if (not locked_present) and (node.locked_id is not None) and (node.last_target_position is not None):
                        try:
                            ...
                            pos_score = max(0.0, 1.0 - (pos_dist / max(0.1, node.relock_max_dist_m)))
                            score += 1.8 * pos_score
                        except Exception:
                            pass

                    if locked_present:
                        # Same ZED id is not absolute identity. During occlusion, ZED can transfer
                        # the old id to a blocker, so we periodically verify appearance and position
                        # before allowing motion commands.
                        if (
                            node._same_id_reid_fail_streak > 0
                            and (node.reid is None or node.target_embedding is None)
                        ):
                            node.get_logger().warn(
                                "Locked ZED id is suspicious and Re-ID is unavailable — waiting for timeout/re-register."
                            )
                            continue
                        ...
                        if node.last_target_position is not None:
                            try:
                                tx, ty, tz = node.last_target_position
                                same_id_jump = math.sqrt((bx - tx) ** 2 + (by - ty) ** 2 + (bz - tz) ** 2)
                                if same_id_jump > node.same_id_max_position_jump_m:
                                    node.get_logger().warn(
                                        f"Locked ZED id jumped {same_id_jump:.2f}m — possible ID transfer, rejecting frame"
                                    )
                                    continue
                            except Exception:
                                pass
                        ...
                        if (
                            ...
                            and (
                                frame_count % node.same_id_reid_check_interval == 0
                                or node._same_id_reid_fail_streak > 0
                            )
                        ):
                            bbox = node._bbox_from_4pts(b.bounding_box_2d)
                            emb = node.reid.extract(frame_bgr, bbox)
                            if emb is None and getattr(node.reid, "_failed", False):
                                node.get_logger().warn(
                                    "OSNet re-ID failed during same-id verification; rejecting frame."
                                )
                                node.reid.close()
                                node.reid = None
                                node.target_embedding = None
                                node._same_id_reid_fail_streak += 1
                                continue
                            elif emb is None:
                                node._same_id_reid_fail_streak += 1
                                continue
                                ...
                                if reid_sim < node.same_id_min_reid:
                                    node._same_id_reid_fail_streak += 1
                                    node.get_logger().warn(
                                        f"Locked ZED id appearance mismatch: re-ID={reid_sim:.2f} "
                                        f"< {node.same_id_min_reid:.2f} — rejecting frame"
                                    )
                                    continue
                                node._same_id_reid_fail_streak = 0
                                score += 0.5 * reid_sim

                    # Re-ID: only apply when NOT actively tracked by ZED.
                    ...
                    if node.reid is not None and node.target_embedding is not None and not locked_present:
                        ...
                        if emb is None and getattr(node.reid, "_failed", False):
                            node.get_logger().warn(
                                "OSNet re-ID failed during relock; switching to color + position fallback."
                            )
                            node.reid.close()
                            node.reid = None
                            node.target_embedding = None
                            if (
                                node.locked_id is not None
                                and node.never_switch_target
                                and (pos_dist is None or pos_dist > node.color_only_relock_max_dist_m)
                            ):
                                continue
                        elif emb is None:
                            continue
                        reid_sim = OsNetReID.similarity(node.target_embedding, emb)
                        if node.reid is not None and reid_sim < node.reid_min_reacquire:
                            continue  # appearance mismatch: not our target person
                        # Re-ID dominates relock ranking; color/position are supporting evidence.
                        if node.reid is not None:
                            score += 5.0 * reid_sim
                            if reid_sim >= node.reid_threshold:
                                score += 0.6
                            if reid_sim >= node.reid_strong_reacquire:
                                score += 1.0
                    elif (
                        node.locked_id is not None
                        and not locked_present
                        and node.never_switch_target
                    ):
                        # No Re-ID available: only relock if still physically near the last target.
                        if pos_dist is None or pos_dist > node.color_only_relock_max_dist_m:
                            continue

                    scored.append((score, ratio, reid_sim, dist, b, poly))

                # ── Rank candidates ──────────────────────────────────────────
                # During Re-ID relock, identity is primary: choose by Re-ID
                # similarity first, then use the composite score only as a tie-break.
                relock_with_reid = (
                    node.locked_id is not None
                    and not locked_present
                ...
                if len(scored) > 0:
                    if relock_with_reid:
                        ...
                        if len(scored) >= 2:
                            reid_gap = scored[0][2] - scored[1][2]
                            if reid_gap < node.reid_discrimination_margin:
                                node.get_logger().warn(
                                    f"Re-ID ambiguous (sim gap {reid_gap:.3f} "
                                    f"< {node.reid_discrimination_margin}) — refusing match"
                                )
                                scored = []
                    else:
                        scored.sort(key=lambda t: t[0], reverse=True)
                        apply_score_margin = (
                            node.target_embedding is not None
                            or (node.locked_id is not None and not locked_present)
                        )
                        if len(scored) >= 2 and apply_score_margin:
                            score_gap = scored[0][0] - scored[1][0]
                            if score_gap < node.relock_score_margin:
                                node.get_logger().warn(
                                    f"Candidate ambiguous (score gap {score_gap:.3f} "
                ...
                if confirm_ok:
                    ...
                    # When confidently tracking the locked ZED id, slowly refresh the
                    # fingerprint so Re-ID adapts to lighting/view-angle changes.
                    if (
                        locked_present
                        and node.reid is not None
                        and node.target_embedding is not None
